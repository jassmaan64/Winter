...
#    return (generateLeft(currState) + generateRight(currState) +
#            generateUp(currState)   + generateDown(currState))

def generateLeft(currState):
	bluh =0
	for rows in range(0, len(currState) - 1):
		for columns in range(0, len(currState) - 1):
			if currState[rows][columns] == 0
				if columns != 0
					bluh = currState[rows][columns-1]
					currState[rows][columns-1]= currState[rows][columns]
					currState[rows][columns]= bluh
	return currState
...
def generateRight(currState):
	...
	for rows in range(0, len(currState) - 1):
		for columns in range(0, len(currState) - 1):
			if currState[rows][columns] == 0
				if columns != len(currState) - 1
					bluh = currState[rows][columns+1]
					currState[rows][columns+1]= currState[rows][columns]
					currState[rows][columns]= bluh
	return currState

def generateUp(currState):
	bluh =0
	for rows in range(0, len(currState) - 1):
		for columns in range(0, len(currState) - 1):
			if currState[rows][columns] == 0
				if rows != 0
					bluh = currState[rows-1][columns]
					currState[rows-1][columns]= currState[rows][columns]
					currState[rows][columns]= bluh
	return currState
