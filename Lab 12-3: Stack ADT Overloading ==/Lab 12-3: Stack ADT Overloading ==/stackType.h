//
//  stackType.h
//  Lab 12-3: Stack ADT Overloading ==
//
//  Created by Jass Maan on 4/16/21.
//
...
{
    if (this == &otherStack) return true;
    if(this->maxStackSize != otherStack.maxStackSize)return false;
    else if(this->stackTop != otherStack.stackTop) return false;
    for(int i=0; i < this->maxStackSize;i++)
        if(this->list[i] != otherStack.list[i])return false;
    return true;
}
