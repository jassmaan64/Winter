...
//IDE Used: X-Code
//Program description: menu-driven program for doubly linked list operations
//*****************
#include <iostream>
#include <cstdlib>
#include <ctime>
...
void outputList(nodeType *current)
{
    ...
    char dll;
    cout << "Output the doubly-linked list (F)orward or (B)ackward? -->";
    cin>>dll;
    dll=tolower(dll);
    while(dll!= 'f' and dll!= 'b')
    {
        cout << "F or B only, please re-enter: ";
        cin>>dll;
        dll=tolower(dll);
    }
